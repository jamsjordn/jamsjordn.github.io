...
function setTheme() {
	...
	if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark')) {
		try {
			window.matchMedia('(prefers-color-scheme: dark').addEventListener('change', () => matchTheme());
		} catch (err) { }
	}
...
function setMenuEvents() {
	...
	function setMenuSelectionEvents() {
		...
		document.querySelectorAll("nav#menu [data-menu-item]").forEach(menuItem => {
	...
	function setHamburgerIconEvents() {
		...
		document.querySelectorAll('header .titleBar .hamburger').forEach(hamburger => {
...
function setTitleStatus() {
	let title = document.querySelectorAll('header .titleBar .title')[0];
	let navHidden = document.querySelectorAll('header nav.hidden');
	let homePage = document.querySelectorAll('main article[data-article="James Jordan"].active section[data-section="Main"].active');
...
function activateMenuItem(menuItem) {
	...
	//Determine Variables
	let sectionName = menuItem.dataset.menuItem;
	let articleName = menuItem.parentNode.querySelectorAll('.header')[0].dataset.menuItem;
	...
	let iChosenArticle = [...menuItems].indexOf(menuItem.parentNode.querySelectorAll('.header')[0]);

	document.querySelectorAll('article').forEach(article => {
		article.classList.remove('active');
		let iCurrentArticle = ([...menuItems].indexOf([...menuItems].find(el => el.dataset.menuItem === article.dataset.article)));
	...
	function slideSections(articleName) {
		document.querySelectorAll(`article[data-article="${articleName}"] section`).forEach(section => {
			let iCurrentSection = ([...menuItems].indexOf([...menuItems].find(el => {
...
function setActiveMenuItem(menuItem) {
	document.querySelectorAll("nav#menu .active").forEach(activeMenuItem => {
